...
llama_kv_cache_unified::llama_kv_cache_unified(const llama_model & model,
...
llama_memory_context_ptr llama_kv_cache_unified::init_batch(llama_batch_allocr & balloc,
                                                            uint32_t             n_ubatch,
                                                            bool                 embd_all) {
    ...
    do {
        ...
        return std::make_unique<llama_kv_cache_unified_context>(this, std::move(sinfos), std::move(ubatches));
    } while (false);
...
bool llama_kv_cache_unified::update(llama_context *          lctx,
                                    bool                     do_shift,
                                    const defrag_info &      dinfo,
                                    const stream_copy_info & sc_info) {
    bool updated = false;
...
ggml_tensor * llama_kv_cache_unified::get_k(ggml_context *    ctx,
                                            int32_t           il,
                                            uint32_t          n_kv,
...
ggml_tensor * llama_kv_cache_unified::get_v(ggml_context *    ctx,
                                            int32_t           il,
                                            uint32_t          n_kv,
                                            const slot_info & sinfo) const {
    const int32_t ikv = map_layer_ids.at(il);
...
ggml_tensor * llama_kv_cache_unified::cpy_k(ggml_context *    ctx,
                                            ggml_tensor *     k_cur,
                                            ggml_tensor *     k_idxs,
                                            int32_t           il,
                                            const slot_info & sinfo) const {
    ...
    ggml_tensor * k_view =
        ggml_view_1d(ctx, k, n_tokens * n_embd_k_gqa, ggml_row_size(k->type, n_embd_k_gqa) * sinfo.head());

    return ggml_cpy(ctx, k_cur, k_view);
}

ggml_tensor * llama_kv_cache_unified::cpy_v(ggml_context *    ctx,
                                            ggml_tensor *     v_cur,
                                            ggml_tensor *     v_idxs,
                                            int32_t           il,
                                            const slot_info & sinfo) const {
    ...
    if (!v_trans) {
        v_view = ggml_view_1d(ctx, v, n_tokens * n_embd_v_gqa, ggml_row_size(v->type, n_embd_v_gqa) * sinfo.head());
    } else {
        v_cur = ggml_transpose(ctx, v_cur);

        v_view = ggml_view_2d(
            ctx, v, n_tokens, n_embd_v_gqa, (v->ne[1]) * ggml_element_size(v), (sinfo.head()) * ggml_element_size(v));
    }
...
void llama_kv_cache_unified::set_input_k_idxs(ggml_tensor *        dst,
...
void llama_kv_cache_unified::set_input_v_idxs(ggml_tensor *        dst,
                                              const llama_ubatch * ubatch,
                                              const slot_info &    sinfo) const {
    if (!supports_set_rows) {
        return;
...
ggml_tensor * llama_kv_cache_unified::build_rope_shift(const llama_cparams & cparams,
                                                       ggml_context *        ctx,
                                                       ggml_tensor *         cur,
                                                       ggml_tensor *         shift,
                                                       ggml_tensor *         factors,
...
ggml_cgraph * llama_kv_cache_unified::build_graph_shift(llm_graph_result * res, llama_context * lctx) const {
    ...
    for (const auto & layer : layers) {
        ...
        ggml_tensor * k = ggml_view_3d(ctx,
                                       layer.k,
                                       n_embd_head_k,
                                       n_head_kv,
                                       get_size() * n_stream,
...
ggml_cgraph * llama_kv_cache_unified::build_graph_defrag(llm_graph_result *  res,
                                                         llama_context *     lctx,
...
bool llama_kv_cache_unified::state_read_meta(llama_io_read_i & io,
                                             uint32_t          strm,
                                             uint32_t          cell_count,
        ...
        for (const auto & layer : layers) {
            ...
            if (cell_count) {
                ...
                ggml_backend_tensor_set(
                    v, io.read(cell_count * v_size_row), head * v_size_row, cell_count * v_size_row);
            }
        }
    } else {
...
llama_kv_cache_unified_context::llama_kv_cache_unified_context(llama_kv_cache_unified * kv) :
...
ggml_tensor * llama_kv_cache_unified_context::cpy_k(ggml_context * ctx,
                                                    ggml_tensor *  k_cur,
                                                    ggml_tensor *  k_idxs,
                                                    int32_t        il) const {
...
ggml_tensor * llama_kv_cache_unified_context::cpy_v(ggml_context * ctx,
                                                    ggml_tensor *  v_cur,
                                                    ggml_tensor *  v_idxs,
                                                    int32_t        il) const {
...
ggml_tensor * llama_kv_cache_unified_context::build_input_k_idxs(ggml_context *       ctx,
                                                                 const llama_ubatch & ubatch) const {
    return kv->build_input_k_idxs(ctx, ubatch);
}

ggml_tensor * llama_kv_cache_unified_context::build_input_v_idxs(ggml_context *       ctx,
                                                                 const llama_ubatch & ubatch) const {
...
void llama_kv_cache_unified_context::set_input_kq_mask(ggml_tensor *        dst,
                                                       const llama_ubatch * ubatch,
